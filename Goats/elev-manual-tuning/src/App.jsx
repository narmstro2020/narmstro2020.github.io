...
function simulateElevator({ motor, numMotors, gearing, drumRadius, carriageMass, ks, kv, ka, kp, kd, kg, setpointM, controlMode, durationS = 5, softLimitLow = 0, softLimitHigh = 2 }) {
    ...
    let position = 0, velocity = 0, prevError = setpointM;
    let appliedVolts = 0, appliedCurrent = 0, motorCurrent = 0;

    for (let i = 0; i <= steps; i++) {
        const t = i * DT;
        const error = setpointM - position;
        ...
        if (controlMode === "voltage") {
            ...
            const netForce = outputForce - gravityForce;
            const accel = carriageMass > 0 ? netForce / carriageMass : 0;
            velocity += accel * DT;
        ...
        if (i % record === 0) {
            history.push({ t, position, positionCm: position * 100, velocity, velocityCmS: velocity * 100, setpointM, setpointCm: setpointM * 100, voltage: appliedVolts, current: appliedCurrent, error, errorCm: error * 100 });
        }
    }
    ...
    if (setpointM > 0) {
        overshoot = ((maxM - setpointM) / setpointM) * 100;
        if (overshoot < 0) overshoot = 0;
    } else if (setpointM < 0) {
        overshoot = ((setpointM - minM) / Math.abs(setpointM)) * 100;
        if (overshoot < 0) overshoot = 0;
    }

    for (let i = history.length - 1; i >= 0; i--) {
        const band = Math.max(Math.abs(setpointM) * 0.02, 0.005);
        if (Math.abs(history[i].position - setpointM) > band) {
...
// ─── Position Chart ─────────────────────────────────────────────────────────
function PositionChart({ history, setpointM }) {
...
export default function ElevatorTunerApp() {
    ...
    const simResult = useMemo(() => {
        return simulateElevator({ motor, numMotors, gearing, drumRadius, carriageMass, ks, kv, ka, kp, kd, kg, setpointM, controlMode, durationS: duration, softLimitLow, softLimitHigh });
    }, [motor, numMotors, gearing, drumRadius, carriageMass, ks, kv, ka, kp, kd, kg, setpointM, controlMode, duration, softLimitLow, softLimitHigh]);
        ...
        <div style={{ minHeight: "100vh", background: "linear-gradient(180deg, #0a0c12 0%, #0f1219 50%, #0a0c12 100%)", color: "#c8ced8", fontFamily: SANS }}>
            ...
            <div style={{ padding: "20px 24px" }}>
                ...
                <div style={{ ...S.cardSubtle, marginBottom: 20 }}>
                    ...
                    {activeTab === "position" && <PositionChart history={simResult.history} setpointM={setpointM} />}
                ...
                {/* Control Equation */}
                <div style={{ ...S.cardSubtle, marginBottom: 20 }}>
                    <div style={S.label}>Control Law — Elevator Position PID</div>
                ...
                <div style={{ ...S.cardSubtle, marginBottom: 20, border: "1px solid rgba(255,255,255,0.05)" }}>
                    ...
                    <div style={{ padding: "12px 16px", background: "rgba(255,255,255,0.02)", borderRadius: 6, border: "1px solid rgba(255,255,255,0.05)", marginBottom: 12 }}>
                        ...
                        <div style={{ fontSize: 12, fontFamily: MONO, opacity: 0.6, lineHeight: 2 }}>
                            ...
                            <div>Net force: <span style={{ color: "#e2e8f0" }}>F_net = F_motor − m·g</span></div>
                    ...
                    <div style={{ padding: "12px 16px", background: "rgba(255,255,255,0.02)", borderRadius: 6, border: "1px solid rgba(255,255,255,0.05)", marginBottom: 12 }}>
                        ...
                        <div style={{ fontSize: 12, fontFamily: MONO, opacity: 0.6, lineHeight: 2 }}>
                            <div>No friction model — real elevators have cable/belt friction and roller resistance</div>
