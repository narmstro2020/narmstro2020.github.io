...
function createHeaderRow() {
    ...
    qualSort.addEventListener("change", () => {
        ...
        let element = document.getElementById("districtSort");
        if (element !== null) {
    ...
    districtSort.addEventListener("change", () => {
        sortByQual = !qualSort.checked;
        sortByDistrictRank = qualSort.checked;
        let element = document.getElementById("qualSort");
        if (element !== null) {
    ...
    table === null || table === void 0 ? void 0 : table.appendChild(headerRow);
}
function createRowContainer() {
    const rows = document.createElement("div");
    rows.classList.add("rows");
    table === null || table === void 0 ? void 0 : table.appendChild(rows);
    return rows;
}
createHeaderRow();
const rowsContainer = createRowContainer();
const toFirst = (teamNumber) => {
...
const incrementRank = (teamNumber) => {
    const teamToUpdate = stateTeams.find(team => team.teamNumber === teamNumber);
    if (teamToUpdate !== undefined) {
        let oldRank = teamToUpdate.qualifyingRank;
        if (oldRank !== 1 || isNaN(oldRank)) {
...
const decrementRank = (teamNumber) => {
    const teamToUpdate = stateTeams.find(team => team.teamNumber === teamNumber);
    if (teamToUpdate !== undefined) {
        let oldRank = teamToUpdate.qualifyingRank;
        if (oldRank !== Team.numberOfTeams || isNaN(oldRank)) {
...
const updateQualRanks = (teamToUpdate, oldRank, newRank) => {
    if (oldRank > newRank) {
...
const updateDrafts = (teamToUpdate, oldDraft, newDraft) => {
...
const updateRows = () => {
    ...
    stateTeams.forEach((team, index) => {
        ...
        rowsContainer === null || rowsContainer === void 0 ? void 0 : rowsContainer.appendChild(newRow);
    });
};
updateRows();
